...
def test_gradebook_bulk_update_and_export(auth_client, app, sample_subjects, sample_students):
    ...
    resp = client.post(f'/students/gradebook/{subj.id}/bulk_update', data=json.dumps(payload), content_type='application/json')
    ...
    # export csv
    resp2 = client.get(f'/students/gradebook/{subj.id}.csv')
