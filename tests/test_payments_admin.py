...
def test_admin_payment_approve_success(admin_client, app):
    ...
    # Action: Approve the payment as admin
    response = admin_client.post(f"/admin/payments/{payment_id}/approve", follow_redirects=True)
...
def test_admin_payment_approve_404(admin_client):
    # Action: Attempt to approve a non-existent payment
    response = admin_client.post("/admin/payments/9999/approve", follow_redirects=True)
...
def test_admin_payment_approve_unauthorized(client, app):
    # Setup: Create a student user and a pending payment
    with app.app_context():
        from werkzeug.security import generate_password_hash
        from models import User
        u = User(username="student_user_approve@example.com", password_hash=generate_password_hash("pass"), role=UserRole.STUDENT)
        db.session.add(u)

        student = Student(first_name="Test", last_name="Student", email="student_user_approve@example.com")
        db.session.add(student)
        db.session.commit()
    ...
    client.post("/login", data={"username": "student_user_approve@example.com", "password": "pass"}, follow_redirects=True)

    # Action: Attempt to approve the payment
    response = client.post(f"/admin/payments/{payment_id}/approve", follow_redirects=True)
